...
const SidebarFilter = ({ pageNumber, dynamicRoute, additionalQuery }) => {
  ...
  const { filterRules, applyFilter, resetFilter, handleOnChangeInputFilter } =
    useFilter(pageNumber, dynamicRoute, additionalQuery);

  const { data: productVariants, isLoading: isLoadingProductsVariants } =
    useProductVariantsData();
  ...
  return (
    <div className={style.sidebarWrapper}>
      <Button
        onClick={handleToggleSidebarMobileFilter}
        className={style.applyFilterMobile}
      >
        <FilterAltIcon fontSize="small" />
      </Button>
      <div
        className={`${style.sidebarWrapperMobile} ${
          sideBarFilterMobile ? style.openSideBarFitlerMobile : ""
        }`}
      ...
      >
        <Button className={style.closeFilterIcon}>
          <CloseIcon
            ...
            fontSize="xs"
            onClick={() => setSideBarFilterMobile(false)}
          />
        </Button>
                ...
                <div className={style.filter}>
                  ...
                  <ul className={`list-unstyled ${style.filterList}`}>
                    {productVariants.brands.map((brand) => {
                      const activeFilterRule = filterRules.brands?.includes(
                        brand.brand_name
                      )
                        ? "checked"
                        : "";
                      ...
                      return (
                        <li key={category.id} className={`${style.filterItem}`}>
                          ...
                          >
                            ...
                            <span>{calcCateogoryProductsCount(category)}</span>
                          </Link>
                        </li>
                      );
                    })}
                ...
                <div className={style.filter}>
                  ...
                  <ul className={`list-unstyled ${style.filterList}`}>
                    {productVariants.sizes.map((size) => {
                      const activeFilterRule = filterRules.sizes.includes(
                        size.size_name
                      )
                        ? "checked"
                        : "";
                      return (
                        <li
                          key={size.id}
                          className={`${style.filterItem} d-flex align-items-center`}
                        >
                ...
                <div className={style.filter}>
                  ...
                  <ul className={`list-unstyled ${style.filterList}`}>
                    {productVariants.colors.map((color) => {
                      const activeFilterColorChecked =
                        filterRules.colors.includes(color.color_name)
                          ? "checked"
                          : "";
                      const activeLabelColorClass = filterRules.colors.includes(
                        color.color_name
                      )
                        ? style.activeColor
                        : "";
