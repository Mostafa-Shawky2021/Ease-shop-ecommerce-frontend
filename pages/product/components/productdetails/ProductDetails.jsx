...
const ProductDetails = ({ productDetails }) => {
  const { addCartData, setProductVariants, productVariants, isLoading } =
    useCarts(productDetails);
  ...
  const handleAddtoCart = (event) => {
    event.stopPropagation();
    setProductVariants((prevState) => ({ ...prevState, quantity: 1 }));
    addCartData();
  };
      ...
      <ul className={`${style.listDetails} list-unstyled`}>
        {!!productDetails?.category_id && (
          <li className={style.item}>
            <span>القسم: </span>
            <Link
              href={`/categoryproducts/${productDetails?.category?.cat_slug}`}
            >
